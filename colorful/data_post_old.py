...
def main(cfg):
    ...
    # 通过标注结果过滤图片，只保留标志的关键帧
    
    label_fix = utility.read_json(os.path.join(cfg["local_path"], cfg["result2label_subdir"], "e2e_recyle_to_fix.json"))
    key_cam_time = []
    for cam_time, val in label_fix["cam_info"].items():
        key_cam_time.append(cam_time)
    ...
    result2label_path = os.path.join(cfg["local_path"], cfg["result2label_subdir"], "e2e_recyle_to_fix.json")
    if os.path.exists(result2label_path):
        post_label_path = os.path.join(data_post_path, "result2label")
        if os.path.exists(post_label_path):
            shutil.rmtree(post_label_path)
        else:
            os.makedirs(post_label_path, exist_ok=True)
        shutil.copy(result2label_path, post_label_path)
